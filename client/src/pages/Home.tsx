...
import { useDeleteProject, useProjects, getCurrentEmail, getCurrentPlan, login } from "@/hooks/use-projects";
import { Button } from "@/components/ui/button";
import { CreateProjectModal } from "@/components/CreateProjectModal";
import { StatusBadge } from "@/components/StatusBadge";
import { Plus, Music, Calendar, Clock, Loader2, ArrowRight, Trash2 } from "lucide-react";
...
export default function Home() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setPlan(result.plan);
      toast({ title: "Logged in", description: `Plan: ${result.plan}` });
    } catch (err) {
      ...
      <div className="container mx-auto px-4 py-12 relative z-10 space-y-8">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="space-y-1 text-center md:text-left">
            ...
            <p className="text-lg text-muted-foreground">Turn music into motion.</p>
          </div>
        ...
        <section className="glass-panel rounded-2xl p-4 md:p-6">
          <div className="flex flex-col md:flex-row gap-3 md:items-center">
            <input
              ...
              placeholder="you@creator.tunivo"
              data-testid="input-email"
            />
            <Button onClick={handleLogin} disabled={isLoggingIn} data-testid="button-login">
            ...
            {projects.map((project) => (
              <Link key={project.id} href={`/project/${project.id}`}>
                <div className="group relative bg-card/40 backdrop-blur-sm border border-white/5 rounded-2xl p-6 hover:border-primary/50 hover:bg-card/60 transition-all duration-300 cursor-pointer overflow-hidden" data-testid={`card-project-${project.id}`}>
                  ...
                  <h3 className="text-xl font-bold font-display mb-2 truncate pr-4 group-hover:text-primary transition-colors">{project.title}</h3>

                  <p className="text-sm text-muted-foreground line-clamp-2 mb-6 h-10">{project.prompt || project.message}</p>
                      ...
                      <Button
                        ...
                        onClick={(e) => {
                          ...
                          deleteProject(project.id, {
                            onSuccess: () => toast({ title: "Deleted", description: "Project removed locally." }),
                          });
                        }}
